...
def pre_work_mkdir(path_photos_from_camera):
    # Create folders to save faces images and csv if they don't exist
    try:
        # Using makedirs to create parent directories as well, if needed
        if not os.path.exists(path_photos_from_camera):
            os.makedirs(path_photos_from_camera, exist_ok=True)  # Avoiding errors if already exists
            print(f"Created directory: {path_photos_from_camera}")
        else:
            print(f"Directory already exists: {path_photos_from_camera}")
    except Exception as e:
        print(f"Error creating directory {path_photos_from_camera}: {e}")
...
def get_faces():
    ...
    if request.method == "POST":
        ...
        with open(current_face_path, "wb") as f:
            f.write(imgdata)
        flag = up.single_pocess(current_face_path)  # 进行上传图片的正确姿势
        if flag != 'right':
            session['num'] -= 1
        ...
        if cid != '' and time != '':
            ...
            for course in courses:
                one_course_records = db.session.query(Attendance).filter(Attendance.s_id == sid,
                                                                         ...
                                                                         Attendance.time.like(time + '%')).order_by(
                    "c_id").all()
                dict[course] = one_course_records
            courses = db.session.query(Course).join(SC).filter(SC.s_id == sid).order_by("c_id").all()
            return render_template('student/my_records.html', dict=dict, courses=courses)
